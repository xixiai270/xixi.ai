...
import { Send, Trash2, MessageSquare, Plus, Sparkles } from 'lucide-react';

export default function XixiAI() {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      // Add XP for each message (25 XP per question)
      addXp(25);
    } catch (error) {
  ...
  const generateImage = async () => {
    ...
    try {
      // Use Claude to generate an image via API
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
      ...
      // Add XP for image generation (50 XP)
      addXp(50);
    } catch (error) {
      console.error('Error:', error);
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: "Désolé, une erreur s'est produite lors de la génération de l'image. 😕"
      }]);
    } finally {
            ...
            <div className="h-full flex flex-col items-center justify-center text-center px-6 py-20">
              <div className="relative mb-6">
                <svg width="180" height="180" viewBox="0 0 180 180" className="drop-shadow-2xl">
                  ...
                  {/* Robot antennas - simple and subtle */}
                  ...
                  {/* White border (Murakami signature thick white outline) */}
